...
- Скопировать все **tasks** из **playbook** в файл `main.yml` директории `/tasks`;
 Перенести все нужные для **tasks** необходимые **handlers** в файл `main.yml`
директории `/handlers`;
- Перенести все **templates** в соответствующую директорию `/role/templates`;
- Все переменные из директорий `defaults` и `vars` для **tasks** необходимо перенести в файлы `defaults/main.yml` , в зависимости от
внутренней логики.

### 9Как создать Role?
Так как мы свою role делаем из playbook, то нам нужно:
- Расширить функционал **tasks**, возможно, использовать несколько уровней вложенности tasks-файлов;
- Заполнить meta информацию, она нужна для работы **ansible-galaxy**;
- Заполнить README.md файл, он нужен пользователям для использования.
...
- Разделение playbook на отдельные role позволяет жить каждой логической единице независимыми жизненными циклами и
...
- Множество готовых role находятся в открытом доступе с лицензиями MIT.
...
`import_tasks` используется в случае, если в задаче есть конкретные переменные
`include_tasks` используется в случае, если в задаче есть переменные, парметры которых берутся из других данных
...
- 01:08:35 - все хендлеры можно внести в один файл `handlers/main.yml`. Их обычно не так много (хендлеров)
...
- прогон `ansible-playbook`
- 01:41:00 - создание директории с новой ролью с помощью `ansible-galaxy`
- 01:42:00 - редактирование роли.
- 01:44:45 - показано как в Git создать новый Relise
