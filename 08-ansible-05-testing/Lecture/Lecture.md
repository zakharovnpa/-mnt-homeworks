...
```
- и потом запустить команду `tox -p5` . !!! Так TOX создаст окружение и оно будет лежать в кеше. Но кэши работают, пока нчего не меняется в конфгурац  наборе версий ансибле, докер  т.д. Поменяется что-нибудь  все, кэш будет создаваться заново.  
