...
#### Для подключения к серверу через CLI необходимо скачать файл конфигурации пользователя с сервера.
* В веб-браузере:
```
https://3.91.145.23:943
```
* Ввести данные авторизации пользователя (не админа)
* В разделе `Available Connection Profiles:` скачать файл профиля по расположенной там ссылке `Yourself (yuser-locked profile)`

![Страница-пользователя](/08-ansible-05-testing/Files/OpenVPN-Access-Server-User-Pages.png)

* Перенести скачанный файл профиля в директорию, где будет выполняться запуск сессии.
...
root@PC-Ubuntu:~# 
```
### Порядок перезапуска ранее остановенного сервера и коррекция его настройки:
1. После перезапуска у сервера поменяется ip адрес.
2. В настройках сервера, в разделе Setings --> Networks  в поле Hostname записать новый ip адрес. Без этого сессия не поднимется.
3. Зайти на страницу пользователя
4. В разделе `Available Connection Profiles:` скачать новый файл профиля по расположенной там ссылке `Yourself (yuser-locked profile)`

![Страница-пользователя](/08-ansible-05-testing/Files/OpenVPN-Access-Server-User-Pages.png)

5. Перенести скачанный файл профиля в директорию, где будет выполняться запуск сессии.
6. Запуск сессии на основании файла-конфигураци пользователя 
