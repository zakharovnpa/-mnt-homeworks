...
#### 5. Для deploy (загрузки артифактов на сревер Nexus) будет необходимо загрузить [settings.xml](./09-ci-05-teamcity/teamcity/settings.xml) в набор конфигураций maven у teamcity, предварительно записав туда креды для подключения к nexus. 
P.S. Креды - это Credentionals - параметры авторизации на сервере - логин и пароль
